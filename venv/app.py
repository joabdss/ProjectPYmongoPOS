...
def lista_pedidos():
    try:
        ...
        for pedido in pedidos:
            ...
            pedidos_serializado.app(pedido)
...
@app.route("/alterarPedido/<id_pedido>", methods=["PUT"])
def update_pedido(id_pedido):
    try:
        ...
        resultado_busca = pedidos_collection.find_one({"id_pedido": id_pedido})
        ...
        if resultado_busca:
            ...
            if resultado_atualizacao.modified_count == 1:
                return f"pedido {id_pedido} atualizado com sucesso.", 200
            else:
                return f"Erro ao atualizar pedido.", 500
        else:
            return f"pedido com id {id_pedido} não encontrado.", 404
    except Exception as e:
        return f"Erro ao atualizar produtos: {e}", 500

@app.route("/excluirPedido/<id_pedido>", methods=["DELETE"])
def delete_pedido(id_pedido):
    try:
        ...
        if resultado_busca:
            ...
            if resultado.deleted_count == 1:
                eturn f"Pedido {id_pedido} excluido com sucesso.", 200
            else:
                return f"Pedido com id {id_pedido} não encontrado.", 404
